...
const posts: Post[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "A cookie closes that gap, because it isn't stuck on one side the way localStorage is. It's written in the browser, but sent to the server with every request after that. So the same Server Component that had no way to check localStorage can check a cookie instead, and render the right theme from the very first byte it sends.",
      "That's the role cookies play here: they're the one piece of client-side information a Server Component is actually allowed to see. Anything stored only in the browser stays invisible to the server until the Client Component runs. A cookie is the bridge.",
    ],
    author: siteConfig.studentName,
    date: "2026-07-09",
